...
if( window == top ){
        ...
        superfish.b.appVersion="12.2.15.34";
...
superfish.b.supportedBy="Similar Products";
superfish.b.shareMsgProd='Similar Products';
...
superfish.b.logoText='Similar Products';
...
//            spsupport.log(">>> items from search >>> " + this.items.length);
		...
		{
			var itemsInView, possibleItemsInView, possibleItemsInMicroView,itemsFromGetData;
			itemsFromGetData = this.items.length;
			itemsInView = (this.items.length > 6) ? 6 : this.items.length;

			possibleItemsInView = Math.floor((this.injectionNode.w-25+24)/65);  // 25 is the more size,24 is the additional space
            this.mode = 'generic'
			if (itemsInView >= possibleItemsInView)
			{
				if (this.displayMode == 'trusty')
				{
					possibleItemsInMicroView = Math.floor((this.injectionNode.w - 26 - 6)/49); // 26 is the more, 6 is for the padding
					if (possibleItemsInMicroView > possibleItemsInView)
					{
						possibleItemsInView = possibleItemsInMicroView;
						this.mode = 'micro';
			...
			if(superfish.b.inImageStats){
			    var alternativeCalculateItems = this.calcPossibleItemsInImage(this.injectionNode.w);

                this.reportAction(
                {
                    action: 'ShowInImage',
                    iiBeforeCareIi: this.calcInImg,
                    realItems: this.items.length,
                    ...
                    realClass: this.mode,
                    imageWidth: this.injectionNode.w,
                    imageHeight: this.injectionNode.h,
                    position: 'inImg'
                });
			}
...
//                shownItems+= "{internalId:" + this.items[i].internalId +
//                " ,itemResType:" + this.items[i].resType +
//                " ,price:" + this.items[i].price +
//                " ,sid:" + this.items[i].sid +
...
//            });
	...
	return {
		...
		possibleItemsInImage: function(img){
		    ...
		    var calcPossibleItemsInImage = inimg.calcPossibleItemsInImage(img.width);
		    return {
                PossibleItemsInImage: calcPossibleItemsInImage.possibleItems,
                itemsInImageClass: calcPossibleItemsInImage.itemsInImageClass
            };
		}
	}
})();
